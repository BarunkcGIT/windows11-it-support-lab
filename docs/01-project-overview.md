# Windows 11 IT Support Lab — Project Overview

## What this project is
A hands-on lab project that simulates a full IT support workflow for a
Windows 11 business device, from hardware readiness through to troubleshooting
and PowerShell automation.

## Why I built it
To practice and demonstrate the core skills required for an IT helpdesk role:
Windows deployment, endpoint security, user management, software deployment,
and technical documentation.

## Phases
1. **Hardware readiness assessment** — verify Windows 11 hardware requirements
2. **Windows 11 installation** — clean install on VMware VM
3. **Post-install configuration** — rename, updates, time zone, user setup
4. **Security baseline** — Defender, Firewall, BitLocker, UAC
5. **Business app deployment** — winget-based silent installation
6. **User and access management** — standard users, least privilege
7. **PowerShell automation** — scripts for readiness, user setup, app install

## Skills demonstrated
- Windows 11 installation and configuration
- Endpoint readiness assessment (TPM, Secure Boot, UEFI)
- PowerShell scripting for system verification and automation
- User and permission management with least privilege
- Windows security configuration
- Software deployment via winget

## Tools used
- VMware Workstation Pro 25H2
- Windows 11 Enterprise Evaluation (24H2, Build 26100)
- PowerShell
- Git and GitHub
- winget (Windows Package Manager)

## Author
IT-tukihenkilö student in Helsinki, building IT support skills through hands-on projects.

LinkedIn: [linkedin.com/in/barunkc](https://linkedin.com/in/barunkc)
